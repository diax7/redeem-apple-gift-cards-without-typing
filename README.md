...
4. On your iPhone or iPad, open the App Store, tap your profile picture, choose **Redeem Gift Card or Code**, then **Use Camera**.
5. Point the camera at a card on your screen, about 4–7 inches (10–18 cm) away. It reads the code immediately.
...
- **The font size is ~34% of the box height**, centered inside the box. At that ratio, sixteen Scancardium glyphs naturally span the full 3:1 width.
...
- The Scancardium font ships with the site in `fonts/Scancardium_2.0.ttf` and is loaded via the [FontFace API](https://developer.mozilla.org/en-US/docs/Web/API/FontFace). The Generate button stays disabled for the brief moment it takes the browser to parse the font — rendering characters before then would briefly use a system font and break the scanner.
